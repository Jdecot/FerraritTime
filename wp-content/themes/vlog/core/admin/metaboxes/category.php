...
if ( !function_exists( 'vlog_save_category_meta_fields' ) ) :
	function vlog_save_category_meta_fields( $term_id ) {
		...
		if ( isset( $_POST['vlog'] ) ) {
			...
			if ( isset( $_POST['vlog']['image'] ) ) {
				$meta['image'] = $_POST['vlog']['image'];
			}
...
if( !function_exists('vlog_category_add_meta_fields') )	: 
	function vlog_category_add_meta_fields() {
			...
			<label><?php esc_html_e( 'Category standard sidebar', 'vlog' ); ?></label>
				<select name="vlog[sidebar][standard_sidebar]" >
					<?php foreach ($all_sidebars as $key => $name) : ?>
						<option value="<?php echo esc_attr($key) ?>" <?php selected( $key, $meta['sidebar']['standard_sidebar'] ); ?>><?php echo esc_html( $name ); ?></option>
					<?php endforeach ?>
				</select>
		</div>
		...
		<div class="form-field vlog-sidebar-opt"> 
			<label><?php esc_html_e( 'Category sticky sidebar', 'vlog' ); ?></label>
				<select name="vlog[sidebar][sticky_sidebar]" >
					<?php foreach ($all_sidebars as $key => $name) : ?>
						<option value="<?php echo esc_attr($key) ?>" <?php selected( $key, $meta['sidebar']['sticky_sidebar'] ); ?>><?php echo esc_html( $name ); ?></option>
					<?php endforeach ?>
				</select>
			</label>	
		</div>
		...
		<div class="form-field">
			<label><?php esc_html_e( 'Image', 'vlog' ); ?></label>
			<?php $display = $meta['image'] ? 'initial' : 'none'; ?>
			<p>
				<img id="vlog-image-preview" src="<?php echo esc_url( $meta['image'] ); ?>" style="display:<?php echo esc_attr( $display ); ?>;">
			</p>

			<p>
				<input type="hidden" name="vlog[image]" id="vlog-image-url" value="<?php echo esc_attr( $meta['image'] ); ?>"/>
				<input type="button" id="vlog-image-upload" class="button-secondary" value="<?php esc_attr_e( 'Upload', 'vlog' ); ?>"/>
				<input type="button" id="vlog-image-clear" class="button-secondary" value="<?php esc_attr_e( 'Clear', 'vlog' ); ?>" style="display:<?php echo esc_attr( $display ); ?>"/>
			</p>

			<p class="description"><?php esc_html_e( 'Upload an image for this category', 'vlog' ); ?></p>
		</div>
...
if ( !function_exists( 'vlog_category_edit_meta_fields' ) ) :
	function vlog_category_edit_meta_fields( $term ) {
		...
		<tr class="form-field">
			<th scope="row" valign="top"><label><?php esc_html_e( 'Image', 'vlog' ); ?></label></th>
			<?php $display = $meta['image'] ? 'initial' : 'none'; ?>
			<td>
			<p>
				<img id="vlog-image-preview" src="<?php echo esc_url( $meta['image'] ); ?>" style="display:<?php echo esc_attr( $display ); ?>;">
