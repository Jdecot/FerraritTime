...
class VLOG_Category_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'vlog_category_widget', 'description' => esc_html__( 'Display your category list with this widget', 'vlog-buddy' ) );
		$control_ops = array( 'id_base' => 'vlog_category_widget' );
		parent::__construct( 'vlog_category_widget', esc_html__( 'Vlog Categories', 'vlog-buddy' ), $widget_ops, $control_ops );

		$this->defaults = array(
			'title' => esc_html__( 'Categories', 'vlog-buddy' ),
			'categories' => array(),
			'count' => 1
		);
	}
	...
	function widget( $args, $instance ) {
		...
		<ul>
		    ...
		    <?php foreach($cats as $cat): ?>
		    	...
		    	<li><a href="<?php echo esc_url(get_category_link($cat)); ?>"><span class="category-text"><?php echo esc_html( $cat->name ); ?></span><?php echo wp_kses_post( $count ); ?></a></li>
		    <?php endforeach; ?> 
		</ul>
